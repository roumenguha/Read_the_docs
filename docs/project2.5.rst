...
Note that the DTYPE struct in this project is almost identical to the `axis_t` struct we used in the multiple DMA lab, here containing a float (`data`) and an int (`last`).
